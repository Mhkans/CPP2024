...
VertexShader::VertexShader(LPCWSTR file_loc, UINT mode, DWORD flags)
{
    ID3DBlob* vertexBlob;
    ...
    // 그러나 이제부터는 셰이더를 한 종류만 사용하는 게 아니기 때문에,
    // 어떤 종류의 셰이더를 쓰냐에 따라 저 용량이 달라지게 되므로 
    // 상황에 따라 다른 값이 계산되어야 한다

    switch (mode) // 이 셰이더가 무슨 정점을 이용하는지에 따라 다르게 계산하도록 스위치
    {
    case 0:
    ...
    case 1:
        ...
        Device::Get()->GetDevice()->CreateInputLayout(layouts_UV, layoutSize, vertexBlob->GetBufferPointer(),
            vertexBlob->GetBufferSize(), &input_layout);
        break;

    case 2:
        layoutSize = ARRAYSIZE(layouts_pos);
        Device::Get()->GetDevice()->CreateInputLayout(layouts_pos, layoutSize, vertexBlob->GetBufferPointer(),
            vertexBlob->GetBufferSize(), &input_layout);
        break;
