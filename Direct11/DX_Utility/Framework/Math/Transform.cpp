	...
	: pos(p), scale(s), rot(Float3(0.0f, 0.0f, a)), parent(parent)
{
	...
	// 단위행렬 : NxN 행렬에서 (k,k) 식의 좌표엔 1이, 나머지 좌표에는 모조리 0이 기록되어있는 행렬
	//			 이 행렬과 곱셈할 경우, 항상 곱하기 전의 행렬이 유지가 되는
	//			 행렬의 곱셈의 항등원

	// 곱셈 등에 이용할 행렬인데 굳이 뭔가 처음부터 초기화할 게 아니라면 단위행렬로 만드는 것이 무난
...
// 이 함수를 통해 이 트랜스폼의 실제 위치를 계산해서 월드 행렬로 만들어둠
// 정확히는, SRT 변환 행렬을 본 함수를 통해 구하여
// 이를 이 오브젝트의 월드 행렬로 삼는 함수
{
	// 본 함수는 트랜스폼이 가지고 있는 데이터들을 토대로
	// 아래의 행렬들을 만들어내어 SRT 변환 행렬을 만들게 됨
	...
	*/
	...
	// 위 둘은 각각 pivot을 이용해서 pivot이 있을 떄를 전제로 하여
	// 추가적으로 변환 행렬을 만든 것
	...
	// SRT 변환 행렬 = 이 오브젝트가 위치한 월드 행렬이 완성됨
	...
	// 아래에서 사용할 추출 함수가 이 3개를 전부 요구하기 떄문에 어쩔 수 없이 선언한 것
