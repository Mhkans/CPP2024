...
SolarSystemScene::SolarSystemScene()
{
	...
	mercury_dummy = new Planet(Vector2(WIN_CENTER_X, WIN_CENTER_Y), Vector2(1, 1), M_PI_2,
		0, 0, 0.0005f);
	mercury = new Planet(Vector2(100.0f, 0), Vector2(1, 1), 0,
		30.0f, 4, 0.0001f);


	venus_dummy = new Planet(Vector2(WIN_CENTER_X, WIN_CENTER_Y), Vector2(1, 1), -M_PI_4,
		0, 0, -0.0004f);
...
void SolarSystemScene::Update()
{
	...
	mercury_dummy->Update();
	mercury->Update(mercury_dummy->GetWorld());

	venus_dummy->Update();
	venus->Update(venus_dummy->GetWorld());
}
