...
void MazePlayer::Update() //플레이어 위치표시
{
    ...
    {
        _time = 0.0f;
        _pos = _path[_pathIndex+1];
        _footPrint = _path[_footPrintIndex];
        _maze->SetBlockType(_pos.y, _pos.x, Block::BlockType::PLAYER);
        _maze->SetBlockType(_footPrint.y, _footPrint.x, Block::BlockType::FOOT_PRINT);
        if (_pos != _endPos) {
            _pathIndex++;
            _footPrintIndex++;
        }
